...
export default function Home() {
  ...
  const properties = PROPERTYLISTINGSAMPLE;

  const filteredProperties = useMemo(() => {
    if (selectedFilters.includes("All")) return properties;

    const hasFilter = (p: PropertyProps, f: string) => {
      const normalized = f.toLowerCase();
      if (normalized === "top villa") return p.rating >= 4.8;
      if (normalized === "self checkin")
        return p.category.some((c) => c.toLowerCase() === "self checkin");
      return p.category.some((c) => c.toLowerCase() === normalized);
    };
